...
HUSD_Imaging::updatePlanes(bool ignore_slapcomp)
{
    ...
    COP_CableStructure outs = mySlapcompProgramManager->getOutputStructure();
    int n_fields = outs.getNumberOfFields();
    for (int i = 0; i < n_fields; i++)
    {
        COP_Type t = outs.getFieldType(i);
        if (t >= COP_TYPE_INT && t <= COP_TYPE_VECTOR4)
        {
            const UT_StringHolder output_name = outs.getFieldName(i);
            mySlapcompPlaneList.append(output_name);
            if (myRendererPlaneList.find(output_name) == -1)
                myPlaneList.append(output_name);
        }
    }
}
                                ...
                                const UT_Vector4F &render_region)
{
    ...
    if(valid)
    {
            ...
            myRenderSettings->prim())
        {
            ...
            if (myRenderSettings->collectAovs(
