        ...
        husd_ImageSaveMap &image_save_map)
{
    ...
    else
    {
        ...
        if (TIL_CopResolver::splitPath(oldpath.c_str(), copnodeid, frame,
            ...
            frame = CHgetFrameFromTime(CHgetEvalTime());
        ...
        husd_ImageSaveMap &image_save_map)
{
	...
	[&layer, &layer_save_path, &output_processors,
         &saved_geo_map, &replace_map,
         &volume_save_map, &image_save_map](const SdfPath &path)
	{
            ...
            else if (primspec)
            {
                ...
                {
                    if (attr->GetTypeName().GetScalarType() ==
                        SdfValueTypeNames->Asset)
                    {
                        ...
                        auto newpath(saveImage(primspec,
                            UsdTimeCode::Default(),
                            attr->GetDefaultValue(), output_processors,
                            layer_save_path, saved_geo_map,
                            image_save_map));
                        if (!newpath.GetAssetPath().empty())
                        {
