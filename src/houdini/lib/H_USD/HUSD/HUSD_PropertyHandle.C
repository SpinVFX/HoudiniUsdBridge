	...
	bool prefix_xform_parms) const
{
    ...
    {
        auto prop_type = parm->getSpareValue(HUSD_PROPERTY_VALUETYPE);
        auto prop_name = parm->getSpareValue(HUSD_PROPERTY_VALUENAME);
	auto ctrlparm = createScriptedControlParm(
                prop_base_name, prop_type, prop_name);

	disablecond = getScriptedControlDisableCondition(
                ctrlparm->myName, prop_type, prop_name);
        ...
        bool prepend_control_parm) const
{
    ...
    UT_StringHolder      attr_type(attr
                            ...
                            : nullptr);
    UT_StringHolder      parm_name = husdGetBaseName( custom_name,
                            attr_name, /*is_xform_op=*/ false );
